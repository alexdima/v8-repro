define(["require", "exports", "assert", "vs/platform/search/common/replace"], function (require, exports, assert, replace_1) {
    /*---------------------------------------------------------------------------------------------
    ...
    suite('Replace Pattern test', function () {
        test('parse replace string', function () {
            var testParse = function (input, expected, expectedHasParameters) {
                var actual = new replace_1.ReplacePattern(input, { pattern: 'somepattern', isRegExp: true });
        ...
        test('create pattern by passing regExp', function () {
            var expected = /abc/;
            var actual = new replace_1.ReplacePattern('hello', false, expected).regExp;
            assert.deepEqual(expected, actual);
            expected = /abc/;
            actual = new replace_1.ReplacePattern('hello', false, /abc/g).regExp;
            assert.deepEqual(expected, actual);
            var testObject = new replace_1.ReplacePattern('hello$0', false, /abc/g);
        ...
        test('get replace string if given text is a complete match', function () {
            var testObject = new replace_1.ReplacePattern('hello', { pattern: 'bla', isRegExp: true });
            var actual = testObject.getReplaceString('bla');
        ...
        test('get replace string for no matches', function () {
            var testObject = new replace_1.ReplacePattern('hello', { pattern: 'bla', isRegExp: true });
            var actual = testObject.getReplaceString('foo');
            assert.equal(null, actual);
            testObject = new replace_1.ReplacePattern('hello', { pattern: 'bla', isRegExp: false });
        ...
        test('get replace string if match is sub-string of the text', function () {
            var testObject = new replace_1.ReplacePattern('hello', { pattern: 'bla', isRegExp: true });
            var actual = testObject.getReplaceString('this is a bla text');
            assert.equal('hello', actual);
            testObject = new replace_1.ReplacePattern('hello', { pattern: 'bla', isRegExp: false });
