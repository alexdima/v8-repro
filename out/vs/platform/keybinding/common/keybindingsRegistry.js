    ...
    var KeybindingsRegistryImpl = /** @class */ (function () {
        ...
        KeybindingsRegistryImpl.prototype.registerKeybindingRule = function (rule) {
            ...
            if (actualKb && actualKb.primary) {
                this._registerDefaultKeybinding(keyCodes_1.createKeybinding(actualKb.primary, platform_1.OS), rule.id, rule.weight, 0, rule.when);
            }
            if (actualKb && Array.isArray(actualKb.secondary)) {
                for (var i = 0, len = actualKb.secondary.length; i < len; i++) {
                    var k = actualKb.secondary[i];
                    this._registerDefaultKeybinding(keyCodes_1.createKeybinding(k, platform_1.OS), rule.id, rule.weight, -i - 1, rule.when);
                }
            }
        };
        ...
        KeybindingsRegistryImpl.prototype._registerDefaultKeybinding = function (keybinding, commandId, weight1, weight2, when) {
            if (platform_1.OS === 1 /* Windows */) {
