...
define(["require", "exports", "assert", "vs/base/common/lifecycle", "vs/base/common/winjs.base", "vs/base/parts/tree/browser/treeModel", "vs/base/parts/tree/browser/treeDefaults", "vs/base/common/event"], function (require, exports, assert, lifecycle, WinJS, model, TreeDefaults, event_1) {
    'use strict';
    Object.defineProperty(exports, "__esModule", { value: true });
    var FakeRenderer = /** @class */ (function () {
    ...
    var TreeModel = /** @class */ (function (_super) {
        ...
        function TreeModel(configuration) {
            return _super.call(this, new TreeContext(configuration)) || this;
        }
        return TreeModel;
    }(model.TreeModel));
    var EventCounter = /** @class */ (function () {
        ...
        EventCounter.prototype.listen = function (event, fn) {
            var _this = this;
            if (fn === void 0) { fn = null; }
            var r = event(function (data) {
                _this._count++;
                if (fn) {
                    fn(data);
                }
    ...
    suite('TreeModel', function () {
        ...
        test('refresh(element, false) refreshes the element', function (done) {
            model.setInput(SAMPLE.AB).then(function () {
                ...
                counter.listen(model.onRefresh); // 1
                counter.listen(model.onDidRefresh); // 1
                counter.listen(model.onDidRefreshItem, function (item) {
    ...
    suite('TreeModel - TreeNavigator', function () {
        ...
        test('last()', function () {
            return model.setInput(SAMPLE.AB).then(function () {
                return model.expandAll([{ id: 'a' }, { id: 'c' }]).then(function () {
                    var nav = model.getNavigator();
    ...
    suite('TreeModel - Expansion', function () {
        ...
        test('collapse, expand', function (done) {
            model.setInput(SAMPLE.AB).done(function () {
                counter.listen(model.onExpandItem, function (e) {
                    assert.equal(e.item.id, 'a');
                    var nav = model.getNavigator(e.item);
                    assert.equal(nav.next() && false, null);
                });
                counter.listen(model.onDidExpandItem, function (e) {
                    assert.equal(e.item.id, 'a');
                    var nav = model.getNavigator(e.item);
                    assert.equal(nav.next().id, 'aa');
                    assert.equal(nav.next().id, 'ab');
        ...
        test('collapseDeepestExpandedLevel', function (done) {
            model.setInput(SAMPLE.DEEP2).done(function () {
                model.expand(SAMPLE.DEEP2.children[0]).done(function () {
                    model.expand(SAMPLE.DEEP2.children[0].children[0]).done(function () {
                        assert(model.isExpanded(SAMPLE.DEEP2.children[0]));
                        assert(model.isExpanded(SAMPLE.DEEP2.children[0].children[0]));
                        model.collapseDeepestExpandedLevel().done(function () {
                            assert(model.isExpanded(SAMPLE.DEEP2.children[0]));
                            assert(!model.isExpanded(SAMPLE.DEEP2.children[0].children[0]));
                            done();
                        });
        ...
        test('expand can trigger refresh', function (done) {
            ...
            model.setInput(SAMPLE.AB).done(function () {
                ...
                assert.equal(nav.next().id, 'c');
                assert.equal(nav.next() && false, null);
                var f = counter.listen(model.onRefreshItemChildren, function (e) {
                    assert.equal(e.item.id, 'a');
                    f();
                });
                var g = counter.listen(model.onDidRefreshItemChildren, function (e) {
                    assert.equal(e.item.id, 'a');
                    g();
                });
                model.expand(SAMPLE.AB.children[0]).done(function () {
        ...
        test('shouldAutoexpand', function (done) {
            // setup
            var model = new TreeModel({
                dataSource: {
                    getId: function (_, e) { return e; },
                    hasChildren: function (_, e) { return true; },
                    ...
                    shouldAutoexpand: function (_, e) { return e === 'b'; }
                }
            });
    ...
    var DynamicModel = /** @class */ (function () {
        ...
        DynamicModel.prototype.getChildren = function (tree, element) {
            ...
            return result.then(function () {
                _this._onDidGetChildren.fire(element);
    ...
    suite('TreeModel - Dynamic data model', function () {
        ...
        test('items get property disposed', function (done) {
            ...
            model.setInput('root').done(function () {
                model.expandAll(['grandfather', 'father', 'son']).done(function () {
                    ...
                    var times = 0;
                    counter.listen(model.onDidDisposeItem, function (item) {
        ...
        test('refreshing grandfather recursively should not refresh collapsed father\'s children immediately', function (done) {
            ...
            model.setInput('root').done(function () {
                ...
                var times = 0;
                var listener = dataModel.onGetChildren(function (element) {
                    times++;
                ...
                model.refresh('grandfather').done(function () {
                    ...
                    listener.dispose();
                    listener = dataModel.onGetChildren(function (element) {
                        times++;
        ...
        test('simultaneously refreshing two disjoint elements should parallelize the refreshes', function (done) {
            ...
            model.setInput('root').done(function () {
                ...
                var getTimes = 0;
                var gotTimes = 0;
                var getListener = dataModel.onGetChildren(function (element) { getTimes++; });
                var gotListener = dataModel.onDidGetChildren(function (element) { gotTimes++; });
        ...
        test('simultaneously recursively refreshing two intersecting elements should concatenate the refreshes - ancestor first', function (done) {
            ...
            model.setInput('root').done(function () {
                ...
                assert.equal(nav.next() && false, null);
                var refreshTimes = 0;
                counter.listen(model.onDidRefreshItem, function (e) { refreshTimes++; });
                var getTimes = 0;
                var getListener = dataModel.onGetChildren(function (element) { getTimes++; });
                var gotTimes = 0;
                var gotListener = dataModel.onDidGetChildren(function (element) { gotTimes++; });
                var p1Completes = [];
                dataModel.promiseFactory = function () { return new WinJS.TPromise(function (c) { p1Completes.push(c); }); };
                ...
                var p2Complete;
                dataModel.promiseFactory = function () { return new WinJS.TPromise(function (c) { p2Complete = c; }); };
        ...
        test('simultaneously recursively refreshing two intersecting elements should concatenate the refreshes - ancestor second', function (done) {
            ...
            model.setInput('root').done(function () {
                ...
                var p2;
                var p1Complete;
                dataModel.promiseFactory = function () { return new WinJS.TPromise(function (c) { p1Complete = c; }); };
                model.refresh('father');
                assert.equal(getTimes, 1);
                assert.equal(gotTimes, 0);
                var p2Completes = [];
                dataModel.promiseFactory = function () { return new WinJS.TPromise(function (c) { p2Completes.push(c); }); };
    ...
    suite('TreeModel - bugs', function () {
        ...
        test('Bug 10566:[tree] build viewlet is broken after some time', function (done) {
            ...
            counter.listen(model.onExpandItem, function () { assert(false, 'should never receive item:expanding event'); });
            counter.listen(model.onDidExpandItem, function () { assert(false, 'should never receive item:expanded event'); });
            model.setInput('root').then(function () {
                // remove bart
