define(["require", "exports", "assert", "vs/base/common/uri", "vs/base/common/paths", "vs/base/common/platform"], function (require, exports, assert, uri_1, paths_1, platform_1) {
    /*---------------------------------------------------------------------------------------------
    ...
    suite('URI', function () {
        ...
        test('URI.file (win-special)', function () {
            if (platform_1.isWindows) {
                assert.equal(uri_1.default.file('c:\\win\\path').toString(), 'file:///c%3A/win/path');
                assert.equal(uri_1.default.file('c:\\win/path').toString(), 'file:///c%3A/win/path');
            }
            else {
                assert.equal(uri_1.default.file('c:\\win\\path').toString(), 'file:///c%3A%5Cwin%5Cpath');
                assert.equal(uri_1.default.file('c:\\win/path').toString(), 'file:///c%3A%5Cwin/path');
            }
        });
        test('file#fsPath (win-special)', function () {
            if (platform_1.isWindows) {
                assert.equal(uri_1.default.file('c:\\win\\path').fsPath, 'c:\\win\\path');
                assert.equal(uri_1.default.file('c:\\win/path').fsPath, 'c:\\win\\path');
                assert.equal(uri_1.default.file('c:/win/path').fsPath, 'c:\\win\\path');
                assert.equal(uri_1.default.file('c:/win/path/').fsPath, 'c:\\win\\path\\');
                assert.equal(uri_1.default.file('C:/win/path').fsPath, 'c:\\win\\path');
        ...
        test('URI#fsPath - no `fsPath` when no `path`', function () {
            var value = uri_1.default.parse('file://%2Fhome%2Fticino%2Fdesktop%2Fcpluscplus%2Ftest.cpp');
        ...
        test('http#toString', function () {
            assert.equal(uri_1.default.from({ scheme: 'http', authority: 'www.msft.com', path: '/my/path' }).toString(), 'http://www.msft.com/my/path');
            assert.equal(uri_1.default.from({ scheme: 'http', authority: 'www.msft.com', path: '/my/path' }).toString(), 'http://www.msft.com/my/path');
            assert.equal(uri_1.default.from({ scheme: 'http', authority: 'www.MSFT.com', path: '/my/path' }).toString(), 'http://www.msft.com/my/path');
            assert.equal(uri_1.default.from({ scheme: 'http', authority: '', path: 'my/path' }).toString(), 'http:my/path');
            assert.equal(uri_1.default.from({ scheme: 'http', authority: '', path: '/my/path' }).toString(), 'http:/my/path');
            assert.equal(uri_1.default.from({ scheme: '', authority: '', path: 'my/path' }).toString(), 'my/path');
            assert.equal(uri_1.default.from({ scheme: '', authority: '', path: '/my/path' }).toString(), '/my/path');
            //http://a-test-site.com/#test=true
            assert.equal(uri_1.default.from({ scheme: 'http', authority: 'a-test-site.com', path: '/', query: 'test=true' }).toString(), 'http://a-test-site.com/?test%3Dtrue');
            assert.equal(uri_1.default.from({ scheme: 'http', authority: 'a-test-site.com', path: '/', query: '', fragment: 'test=true' }).toString(), 'http://a-test-site.com/#test%3Dtrue');
        });
        test('http#toString, encode=FALSE', function () {
            assert.equal(uri_1.default.from({ scheme: 'http', authority: 'a-test-site.com', path: '/', query: 'test=true' }).toString(true), 'http://a-test-site.com/?test=true');
            assert.equal(uri_1.default.from({ scheme: 'http', authority: 'a-test-site.com', path: '/', query: '', fragment: 'test=true' }).toString(true), 'http://a-test-site.com/#test=true');
            assert.equal(uri_1.default.from({}).with({ scheme: 'http', path: '/api/files/test.me', query: 't=1234' }).toString(true), 'http:/api/files/test.me?t=1234');
        ...
        test('with, identity', function () {
            var uri = uri_1.default.parse('foo:bar/path');
            var uri2 = uri.with(null);
            assert.ok(uri === uri2);
        ...
        test('with, changes', function () {
            assert.equal(uri_1.default.parse('before:some/file/path').with({ scheme: 'after' }).toString(), 'after:some/file/path');
            assert.equal(uri_1.default.from({}).with({ scheme: 'http', path: '/api/files/test.me', query: 't=1234' }).toString(), 'http:/api/files/test.me?t%3D1234');
            assert.equal(uri_1.default.from({}).with({ scheme: 'http', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' }).toString(), 'http:/api/files/test.me?t%3D1234');
            assert.equal(uri_1.default.from({}).with({ scheme: 'https', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' }).toString(), 'https:/api/files/test.me?t%3D1234');
            assert.equal(uri_1.default.from({}).with({ scheme: 'HTTP', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' }).toString(), 'HTTP:/api/files/test.me?t%3D1234');
            assert.equal(uri_1.default.from({}).with({ scheme: 'HTTPS', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' }).toString(), 'HTTPS:/api/files/test.me?t%3D1234');
            assert.equal(uri_1.default.from({}).with({ scheme: 'boo', authority: '', path: '/api/files/test.me', query: 't=1234', fragment: '' }).toString(), 'boo:/api/files/test.me?t%3D1234');
        });
        test('with, remove components #8465', function () {
            assert.equal(uri_1.default.parse('scheme://authority/path').with({ authority: '' }).toString(), 'scheme:/path');
            assert.equal(uri_1.default.parse('scheme:/path').with({ authority: 'authority' }).with({ authority: '' }).toString(), 'scheme:/path');
            assert.equal(uri_1.default.parse('scheme:/path').with({ authority: 'authority' }).with({ authority: null }).toString(), 'scheme:/path');
            assert.equal(uri_1.default.parse('scheme:/path').with({ authority: 'authority' }).with({ path: '' }).toString(), 'scheme://authority');
            assert.equal(uri_1.default.parse('scheme:/path').with({ authority: 'authority' }).with({ path: null }).toString(), 'scheme://authority');
            assert.equal(uri_1.default.parse('scheme:/path').with({ authority: '' }).toString(), 'scheme:/path');
            assert.equal(uri_1.default.parse('scheme:/path').with({ authority: null }).toString(), 'scheme:/path');
        });
        test('with, validation', function () {
            var uri = uri_1.default.parse('foo:bar/path');
            assert.throws(function () { return uri.with({ scheme: 'fai:l' }); });
            assert.throws(function () { return uri.with({ scheme: 'fäil' }); });
            assert.throws(function () { return uri.with({ authority: 'fail' }); });
            assert.throws(function () { return uri.with({ path: '//fail' }); });
        });
        test('parse', function () {
            ...
            value = uri_1.default.parse('f3ile:#d');
            assert.equal(value.scheme, 'f3ile');
            assert.equal(value.authority, '');
            assert.equal(value.path, '');
            assert.equal(value.query, '');
            assert.equal(value.fragment, 'd');
            value = uri_1.default.parse('foo+bar:path');
        ...
        test('URI#file, win-speciale', function () {
            if (platform_1.isWindows) {
                ...
                value = uri_1.default.file('\\\\shäres\\path\\c#\\plugin.json');
                assert.equal(value.scheme, 'file');
        ...
        test('VSCode URI module\'s driveLetterPath regex is incorrect, #32961', function () {
            var uri = uri_1.default.parse('file:///_:/path');
            assert.equal(uri.fsPath, platform_1.isWindows ? '\\_:\\path' : '/_:/path');
        });
        test('URI#file, no path-is-uri check', function () {
            // we don't complain here
            var value = uri_1.default.file('file://path/to/file');
            assert.equal(value.scheme, 'file');
            assert.equal(value.authority, '');
        ...
        test('URI#toString, lower-case windows drive letter', function () {
            assert.equal(uri_1.default.parse('untitled:c:/Users/jrieken/Code/abc.txt').toString(), 'untitled:c%3A/Users/jrieken/Code/abc.txt');
            assert.equal(uri_1.default.parse('untitled:C:/Users/jrieken/Code/abc.txt').toString(), 'untitled:c%3A/Users/jrieken/Code/abc.txt');
        });
        ...
        test('URI#toString, don\'t encode port', function () {
            ...
            value = uri_1.default.from({ scheme: 'http', authority: 'löcalhost:8080', path: '/far', query: undefined, fragment: undefined });
            assert.equal(value.toString(), 'http://l%C3%B6calhost:8080/far');
        });
        test('URI#toString, user information in authority', function () {
            var value = uri_1.default.parse('http://foo:bar@localhost/far');
            assert.equal(value.toString(), 'http://foo:bar@localhost/far');
            value = uri_1.default.parse('http://foo@localhost/far');
            assert.equal(value.toString(), 'http://foo@localhost/far');
            value = uri_1.default.parse('http://foo:bAr@localhost:8080/far');
            assert.equal(value.toString(), 'http://foo:bAr@localhost:8080/far');
            value = uri_1.default.parse('http://foo@localhost:8080/far');
            assert.equal(value.toString(), 'http://foo@localhost:8080/far');
            value = uri_1.default.from({ scheme: 'http', authority: 'föö:bör@löcalhost:8080', path: '/far', query: undefined, fragment: undefined });
        ...
        test('URI - http, query & toString', function () {
            ...
            assert.equal(uri2.query, uri.query);
            // #24849
            uri = uri_1.default.parse('https://twitter.com/search?src=typd&q=%23tag');
