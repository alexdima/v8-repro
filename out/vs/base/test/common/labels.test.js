 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/base/common/labels", "vs/base/common/platform", "vs/base/common/labels"], function (require, exports, assert, labels, platform, labels_1) {
    'use strict';
    Object.defineProperty(exports, "__esModule", { value: true });
    suite('Labels', function () {
        test('shorten - windows', function () {
        ...
        test('shorten - not windows', function () {
            if (platform.isWindows) {
                assert.ok(true);
                return;
            }
        ...
        test('template', function () {
            ...
            var t = '${activeEditorShort}${separator}${rootName}';
        ...
        test('getBaseLabel - unix', function () {
            if (platform.isWindows) {
                assert.ok(true);
                return;
            }
            assert.equal(labels_1.getBaseLabel('/some/folder/file.txt'), 'file.txt');
            assert.equal(labels_1.getBaseLabel('/some/folder'), 'folder');
            assert.equal(labels_1.getBaseLabel('/'), '/');
        });
        test('getBaseLabel - windows', function () {
            if (!platform.isWindows) {
                assert.ok(true);
                return;
            }
            assert.equal(labels_1.getBaseLabel('c:'), 'C:');
            assert.equal(labels_1.getBaseLabel('c:\\'), 'C:');
            assert.equal(labels_1.getBaseLabel('c:\\some\\folder\\file.txt'), 'file.txt');
            assert.equal(labels_1.getBaseLabel('c:\\some\\folder'), 'folder');
        });
    });
});
