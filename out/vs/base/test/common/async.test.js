define(["require", "exports", "assert", "vs/base/common/winjs.base", "vs/base/common/async", "vs/base/common/uri"], function (require, exports, assert, winjs_base_1, Async, uri_1) {
    /*---------------------------------------------------------------------------------------------
    ...
    suite('Async', function () {
        ...
        test('Throttler - progress should work', function (done) {
            var order = 0;
            var factory = function () { return new winjs_base_1.TPromise(function (c, e, p) {
        ...
        test('Queue - simple', function (done) {
            var queue = new Async.Queue();
            var syncPromise = false;
            var f1 = function () { return winjs_base_1.TPromise.as(true).then(function () { return syncPromise = true; }); };
            var asyncPromise = false;
            var f2 = function () { return winjs_base_1.TPromise.timeout(10).then(function () { return asyncPromise = true; }); };
            assert.equal(queue.size, 0);
            queue.queue(f1);
            assert.equal(queue.size, 0); // sync promise is already done
            var p = queue.queue(f2);
        ...
        test('Queue - order is kept', function (done) {
            var queue = new Async.Queue();
            var res = [];
            var f1 = function () { return winjs_base_1.TPromise.as(true).then(function () { return res.push(1); }); };
            var f2 = function () { return winjs_base_1.TPromise.timeout(10).then(function () { return res.push(2); }); };
            var f3 = function () { return winjs_base_1.TPromise.as(true).then(function () { return res.push(3); }); };
            var f4 = function () { return winjs_base_1.TPromise.timeout(20).then(function () { return res.push(4); }); };
            var f5 = function () { return winjs_base_1.TPromise.timeout(0).then(function () { return res.push(5); }); };
            queue.queue(f1);
            queue.queue(f2);
            queue.queue(f3);
            queue.queue(f4);
            queue.queue(f5).then(function () {
        ...
        test('Queue - errors bubble individually but not cause stop', function (done) {
            ...
            var f1 = function () { return winjs_base_1.TPromise.as(true).then(function () { return res.push(1); }); };
            var f2 = function () { return winjs_base_1.TPromise.timeout(10).then(function () { return res.push(2); }); };
            var f3 = function () { return winjs_base_1.TPromise.as(true).then(function () { return winjs_base_1.TPromise.wrapError(new Error('error')); }); };
            var f4 = function () { return winjs_base_1.TPromise.timeout(20).then(function () { return res.push(4); }); };
            var f5 = function () { return winjs_base_1.TPromise.timeout(0).then(function () { return res.push(5); }); };
            queue.queue(f1);
            queue.queue(f2);
            queue.queue(f3).then(null, function () { return error = true; });
            queue.queue(f4);
            queue.queue(f5).then(function () {
                assert.equal(res[0], 1);
                assert.equal(res[1], 2);
        ...
        test('Queue - order is kept (chained)', function (done) {
            ...
            var f4 = function () { return winjs_base_1.TPromise.timeout(20).then(function () { return res.push(4); }); };
            var f5 = function () { return winjs_base_1.TPromise.timeout(0).then(function () { return res.push(5); }); };
            queue.queue(f1).then(function () {
                queue.queue(f2).then(function () {
                    queue.queue(f3).then(function () {
                        queue.queue(f4).then(function () {
                            queue.queue(f5).then(function () {
                                assert.equal(res[0], 1);
                                assert.equal(res[1], 2);
                                assert.equal(res[2], 3);
        ...
        test('Queue - events', function (done) {
            var queue = new Async.Queue();
            var finished = false;
            queue.onFinished(function () {
                done();
            });
            var res = [];
            var f1 = function () { return winjs_base_1.TPromise.timeout(10).then(function () { return res.push(2); }); };
            var f2 = function () { return winjs_base_1.TPromise.timeout(20).then(function () { return res.push(4); }); };
            var f3 = function () { return winjs_base_1.TPromise.timeout(0).then(function () { return res.push(5); }); };
            var q1 = queue.queue(f1);
            var q2 = queue.queue(f2);
            queue.queue(f3);
            q1.then(function () {
                assert.ok(!finished);
                q2.then(function () {
        ...
        test('ResourceQueue - simple', function () {
            var queue = new Async.ResourceQueue();
            var r1Queue = queue.queueFor(uri_1.default.file('/some/path'));
            var r2Queue = queue.queueFor(uri_1.default.file('/some/other/path'));
            assert.ok(r1Queue);
            assert.ok(r2Queue);
            assert.equal(r1Queue, queue.queueFor(uri_1.default.file('/some/path'))); // same queue returned
            var syncPromiseFactory = function () { return winjs_base_1.TPromise.as(true); };
            r1Queue.queue(syncPromiseFactory);
            var r1Queue2 = queue.queueFor(uri_1.default.file('/some/path'));
            assert.notEqual(r1Queue, r1Queue2); // previous one got disposed after finishing
        });
        test('ThrottledEmitter', function () {
            var emitter = new Async.ThrottledEmitter();
            var fnThatEmitsEvent = function () {
                emitter.fire();
            };
            var promiseFn = winjs_base_1.TPromise.timeout(0).then(function () {
                fnThatEmitsEvent();
                fnThatEmitsEvent();
                fnThatEmitsEvent();
            });
