define(["require", "exports", "vs/editor/common/model", "vs/editor/common/model/chunksTextBuffer/bufferPiece", "vs/editor/common/core/position", "vs/editor/common/core/range", "vs/base/common/strings"], function (require, exports, model_1, bufferPiece_1, position_1, range_1, strings) {
    /*---------------------------------------------------------------------------------------------
    ...
    var ChunksTextBuffer = /** @class */ (function () {
        ...
        ChunksTextBuffer.prototype.applyEdits = function (rawOperations, recordTrimAutoWhitespace) {
            if (rawOperations.length === 0) {
                return new model_1.ApplyEditsResult([], [], []);
            }
            ...
            for (var i = 0; i < rawOperations.length; i++) {
                ...
                operations[i] = {
                    ...
                    rangeLength: this.getValueLengthInRange(validatedRange, model_1.EndOfLinePreference.TextDefined),
            ...
            for (var i = 0; i < operations.length; i++) {
                ...
                reverseOperations[i] = {
                    ...
                    range: reverseRange,
                    text: this.getValueInRange(op.range, model_1.EndOfLinePreference.TextDefined),
    ...
    exports.ChunksTextBuffer = ChunksTextBuffer;
    ...
    var Buffer = /** @class */ (function () {
        ...
        Buffer.prototype._flushLeafEdits = function (accumulatedLeafIndex, accumulatedLeafEdits, replacements) {
            if (accumulatedLeafEdits.length > 0) {
                ...
                bufferPiece_1.BufferPiece.replaceOffsetLen(this._leafs[accumulatedLeafIndex], accumulatedLeafEdits, this._idealLeafLength, this._maxLeafLength, rep.replacements);
            }
            accumulatedLeafEdits.length = 0;
        };
        Buffer.prototype._pushLeafEdits = function (start, length, text, accumulatedLeafEdits) {
            if (length !== 0 || text.length !== 0) {
                accumulatedLeafEdits.push(new bufferPiece_1.LeafOffsetLenEdit(start, length, text));
            }
        };
        Buffer.prototype._appendLeaf = function (leaf, leafs, prevLeaf) {
            ...
            if ((prevLeafLength < this._minLeafLength || currLeafLength < this._minLeafLength) && prevLeafLength + currLeafLength <= this._maxLeafLength) {
                var joinedLeaf = bufferPiece_1.BufferPiece.join(prevLeaf, leaf);
            ...
            if ((lastChar >= 0xd800 && lastChar <= 0xdbff) || (lastChar === 13 /* CarriageReturn */ && firstChar === 10 /* LineFeed */)) {
                var modifiedPrevLeaf = bufferPiece_1.BufferPiece.deleteLastChar(prevLeaf);
                leafs[leafs.length - 1] = modifiedPrevLeaf;
                var modifiedLeaf = bufferPiece_1.BufferPiece.insertFirstChar(leaf, lastChar);
                leaf = modifiedLeaf;
            }
            leafs.push(leaf);
            prevLeaf = leaf;
        ...
        Buffer.prototype.replaceOffsetLen = function (_edits) {
            ...
            if (leafs.length === 0) {
                ...
                leafs.push(new bufferPiece_1.BufferPiece(''));
            }
            this._leafs = leafs;
            this._rebuildNodes();
        };
        Buffer.prototype.setEOL = function (newEOL) {
            ...
            for (var i = 0, len = this._leafs.length; i < len; i++) {
                leafs[i] = bufferPiece_1.BufferPiece.normalizeEOL(this._leafs[i], newEOL);
            }
            this._leafs = leafs;
            this._rebuildNodes();
            this._eol = newEOL;
